...
`protocols/gate-run.parley` (generated by `observer/gen_gate_protocol.py`)
is the gate flow as actually practiced. Against 229 real runs:

```
229 traces: 155 complete, 72 stalled, 2 deviating
```

The 72 stalled are parked runs awaiting a judge/resolution (mostly
...
instead of a judge; ceiling parks nonetheless re-judged; one run that
looped the judge four times. And it exposed the algebra's limits — the
judge loop must be unrolled (744 generated lines that recursion would
express in ~20), and "judgment or resolution" wants a choice between
...
- **Recursion in the algebra** — the gate observer's unrolled loop is the
  standing argument; evidence in `protocols/gate-run.parley`.
...
- Live observer: tail gate's log and flag deviations as they append.
